package eu.virtusdevelops.simplemachines.utils.NBT;
...
public class NBTUtil {
    ...
    public NBTUtil(JavaPlugin plugin){
        this.plugin = plugin;
    }

    public Integer getInt(ItemStack item, String dataContainer){
        NamespacedKey key = new NamespacedKey(plugin, dataContainer);
    ...
    /*public ItemStack createBeacon(ItemStack item, int level){
        try {
            ...
            meta.setDisplayName(HexUtil.colorify(plugin.getFileManager().getConfiguration("beacons").getString("beacons." + level + ".name")));
